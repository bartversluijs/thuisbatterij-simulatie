...
async function handleFormSubmit(e) {
    ...
    // Phase 2: staged mode → effective efficiency = battery × inverter per direction (else passthrough).
...
function displayResults(results, year, simulator) {
    ...
    // Throughput & equivalent full cycles (Phase 5) — from the with-battery run.
    const tp = withBattery.throughput;
...
function loadParametersFromUrl() {
    ...
    if (params.has('efficiencyStaged')) {
        ...
        if (stagedEl) {
            ...
            stagedEl.dispatchEvent(new Event('change'));
        }
    }
    ...
    if (params.has('partLoadEnabled')) {
        ...
        if (plEl) {
            ...
            plEl.dispatchEvent(new Event('change'));
        }
...
function updateUrlWithParameters() {
    ...
    // Staged efficiency (Phase 2) — only persist when enabled.
    const stagedEl = document.getElementById('efficiencyStaged');
    ...
    // Part-load efficiency (Phase 3) — only persist when enabled.
